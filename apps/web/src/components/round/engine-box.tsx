...
export function EngineBox() {
	const { config, history, cursor, turn, status, cells } = useGameStore(
		useShallow((s) => ({ config: s.config, history: s.history, cursor: s.cursor, turn: s.turn, status: s.status, cells: s.cells })),
	);
	...
	useEffect(() => {
		...
		let stale = false;
		const t = setTimeout(() => {
			requestAnalysis(config, history.slice(0, cursor), 4).then((analysis) => !stale && setResult({ key, analysis }));
		}, 250);
		return () => {
			stale = true;
			clearTimeout(t);
	...
	const label = !a ? '…' : Math.abs(white) >= MATE ? (white > 0 ? 'White wins' : 'Black wins') : `${white >= 0 ? '+' : '−'}${(Math.abs(white) / 100).toFixed(1)}`;
	...
	return (
		<div className="box p-3 text-sm flex flex-col gap-2">
			<div className="flex items-center justify-between">
				...
				<span className={cn('font-mono font-semibold', status.kind !== 'playing' && 'text-muted-foreground')}>
			...
			{a && status.kind === 'playing' && (
				<div className="flex items-center justify-between gap-2">
					<span className="text-xs text-muted-foreground">
						Best: <span className="font-mono text-foreground">{notation(a.move, cells[a.move.from]?.piece?.kind)}</span>
					</span>
