...
analysis board with branching + engine eval, board editor (custom positions, also for friend challenges), interactive piece tutorial, play vs computer (4 levels, in a worker), 3D cube or unfolded 2D net, move list w/ keyboard nav, zen mode,
...
Browser flows (need playwright + chromium, dev stack running): `PW=<playwright/index.mjs> CHROME=<chrome> node apps/web/e2e/flow.mjs` (also `e2e/tournament.mjs`, `e2e/editor.mjs`).
