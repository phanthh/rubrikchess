...
Phase 16 as implemented: `users.last_rated_at`; `rating::inflate` runs in `room::rate` before `update` (overall rating only). IP limits use `client_ip` (peer, or first `X-Forwarded-For` hop when `TRUST_PROXY=1`); router served with `into_make_service_with_connect_info`. Arena chat: `{t:"tour_sub", id}` / `{t:"tour_unsub"}` (one per socket); `tour_chat` only reaches subscribers; the arena page subscribes on mount/reconnect.
